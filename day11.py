...
L.LLLLLL.L
L.LLLLL.LL'''
inputs = '''LLLLL.LLLLLLL.LLLLLL.L.LLLL..LLLL.LLLLLLLLL.LLLLLLLL.LLLLLLLLLLLLL.L.LLLLLLLLLLLLLL.LLLLLL
...
# approach 1: make a wall of floor seats around the array to eliminate the oob issue 
#          1b: simply check so it will not go oob
# approach 2: use math? Matrix???? Matrix related maths???
            ...
            for x in [-1, 0, 1]:
                ...
                for y in [-1, 0, 1]:
                    ...
                    while result[row][col] == '.':
                        if row > 0 and col > 0 and row < len(result) - 1 and col < len(result[i]) - 1:
                            row += x
                            col += y
                        else:
                            break                      
                    ...
                    # done evaluating nearby space
            if result[i][j] == 'L' and count == 0:
                temp[i][j] = '#'
                change = True
            elif result[i][j] == '#' and count >= 5: # part 1 change
                temp[i][j] = 'L'
                change = True
    #print(*temp, sep ='\n')
    #print('_'*50)
